...
def add_film(film_name: str, film_year: int) -> None:
    with closing(conn.cursor()) as cur:
        cur.execute('INSERT OR IGNORE INTO films (name, year) values (?, ?)', (film_name, film_year,))
    conn.commit()


def add_showing(film_name: str, film_year: int, chain_name: str, cinema_name: str, screen_name: str, time: datetime, json_attributes: str) -> None:
...
def create_table() -> None:
    with closing(conn.cursor()) as cur:
        ...
        cur.execute('CREATE TABLE IF NOT EXISTS films (name TEXT, year INT, PRIMARY KEY (name, year))')
        cur.execute('CREATE TABLE IF NOT EXISTS showings (filmName TEXT, filmYear INT, chainName TEXT, cinemaName TEXT, screenName TEXT, utc_time INT, jsonAttributes TEXT, PRIMARY KEY (filmName, filmYear, chainName, cinemaName, screenName, utc_time), FOREIGN KEY (filmName) REFERENCES films (name), FOREIGN KEY (filmYear) REFERENCES films (year), FOREIGN KEY (chainName) REFERENCES screens (chainName), FOREIGN KEY (cinemaName) REFERENCES screens (cinemaName), FOREIGN KEY (screenName) REFERENCES screens (name))')
